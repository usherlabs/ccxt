...
var verityClient = require('@usherlabs/verity-client');
...
class Exchange {
    constructor(userConfig = {}) {
        ...
        this.timeout = 90000; // milliseconds
        this.verbose = false;
    ...
    isLoadMarketsRequest(url, method) {
        // Check if this request is related to loading markets
        // This includes common market-related endpoints that loadMarkets might call
        ...
        return marketEndpoints.some(endpoint => lowerUrl.includes(endpoint.toLowerCase()));
    }
    async fetch(url, method = 'GET', headers = undefined, body = undefined) {
        ...
        try {
            // Exception for loadMarkets method - always use axios, never VerityClient
            const isLoadMarketsRequest = this.isLoadMarketsRequest(url, method);
            if (!this.useVerity || isLoadMarketsRequest) {
                const response = await axios__default["default"](axiosConfig);
                return this.handleRestResponse(response, url, method, headers, body);
            }
            else {
                const client = new verityClient.VerityClient({ prover_url: this.verityProverUrl });
            ...
            if (axios__default["default"].isAxiosError(error)) {
                if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
                    ...
                    throw new errors.NetworkError(this.id + ' ' + method + ' ' + url + ' network error: ' + error.message);
                }
                else {
                    throw new errors.NetworkError(this.id + ' ' + method + ' ' + url + ' request failed: ' + error.message);
                }
            }
            throw error;
        }
    ...
    async loadMarkets(reload = false, params = {}) {
        ...
        // Load urlToMethodMap only in Node.js environment
        if (isNode) {
            const fsModule = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(/* webpackIgnore: true */ 'node:fs')); });
            try {
                const rawData = fsModule.readFileSync(`../../src/test/static/request/${this.id}.json`, 'utf8');
                const config = JSON.parse(rawData);
                this.urlToMethodMap = this.buildFlatUrlMethodMap(config.methods);
            }
            catch (error) {
                // If file doesn't exist or can't be read, continue without urlToMethodMap
                const errorMessage = error instanceof Error ? error.message : String(error);
                this.log(`Could not load urlToMethodMap for ${this.id}: ${errorMessage}`);
            }
        }
        // In browser environment, urlToMethodMap remains empty object
        return this.marketsLoading;
    }
    buildFlatUrlMethodMap(methods) {
        const result = {};
